...
// 🆕 Active + locked terms
export const ACTIVE_TERM = "Spring 2025-26";
export const LOCKED_TERMS = ["Winter 2025-26"];
...
export async function castVote(periodLabel, uid, newTruckId) {
  // 🔒 Block voting on locked terms
  if (LOCKED_TERMS.includes(periodLabel)) {
    throw new Error("This term is locked. Voting is closed.");
  }
  ...
  const updatedVotes = await runTransaction(db, async (tx) => {
    ...
    const votes = periodSnap.exists()
      ? { ...(periodSnap.data().votes ?? {}) }
      : {};

    const previousTruckId = userSnap.exists()
      ? userSnap.data().truckId
      : null;

    if (previousTruckId === newTruckId) return votes;

    if (previousTruckId) {
      votes[previousTruckId] = Math.max(
        0,
        (votes[previousTruckId] ?? 0) - 1
      );
    }
