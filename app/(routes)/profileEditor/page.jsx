...
export default function Page() {
  ...
  const [foodCaption, setFoodCaption] = useState("");
  ...
  const foodWordCount = countWords(foodCaption);
  ...
  async function loadFoodPosts(userId) {
    try {
      ...
      console.error("Failed to load food posts:", err);
      setFoodPosts([]);
    }
  }
  ...
  const handleFoodPostUpload = async () => {
    if (!uid) {
      alert("You must be logged in to upload food posts.");
      return;
    }

    if (!foodImageFile) {
      alert("Please choose a food photo first.");
      return;
    }
    ...
    if (foodWordCount > 100) {
      alert("Caption must stay under 100 words.");
      return;
    }

    const MAX_MB = 5;
    if (foodImageFile.size > MAX_MB * 1024 * 1024) {
      alert(`Food image too large. Please upload an image under ${MAX_MB}MB.`);
      return;
    }
    ...
    try {
      const safeName = `${Date.now()}-${foodImageFile.name.replace(/\s+/g, "-")}`;
      const fileRef = ref(storage, `foods/${uid}/${safeName}`);

      await uploadBytes(fileRef, foodImageFile);
      const imageUrl = await getDownloadURL(fileRef);

      await addDoc(collection(db, "users", uid, "foods"), {
        imageUrl,
        caption: foodCaption.trim(),
        createdAt: serverTimestamp(),
      });

      setFoodCaption("");
      setFoodImageFile(null);
      await loadFoodPosts(uid);
      alert("Food post uploaded!");
    } catch (err) {
      console.error("Food post upload failed:", err);
      alert("Food post upload failed — check console.");
    } finally {
      setUploadingFood(false);
    }
  };

  const handleDeleteFoodPost = async (postId) => {
    if (!uid) {
      alert("You must be logged in to delete posts.");
      return;
    }

    const confirmed = window.confirm("Delete this food post?");
    if (!confirmed) return;

    try {
      await deleteDoc(doc(db, "users", uid, "foods", postId));
      setFoodPosts((prev) => prev.filter((post) => post.id !== postId));
    } catch (err) {
      console.error("Failed to delete food post:", err);
  ...
  return (
    <div className={`${styles.app} ${isDark ? styles.dark : ""}`}>
      <div className={styles.page}>
        ...
        <section className={`${styles.card} ${styles.previewCard}`}>
          ...
          <div className={styles.previewBody}>
            ...
            <div className={styles.previewStats}>
              <p>Food Posts: {foodPosts.length}</p>
            </div>
          </div>
        </section>
        ...
        <section className={styles.card}>
          <h2 className={styles.cardTitle}>Foods I’ve Eaten</h2>
          <p className={styles.cardSubtitle}>
            Upload a food photo and add a caption under 100 words.
          </p>

          <div className={styles.field}>
            <label>Food Photo</label>
            <input type="file" accept="image/*" onChange={handleFoodFileChange} />
            {foodImageFile ? <p className={styles.helper}>Selected: {foodImageFile.name}</p> : null}
          </div>
          ...
          <div className={styles.field}>
            ...
            <textarea
              rows={3}
              value={foodCaption}
              onChange={(e) => setFoodCaption(e.target.value)}
              placeholder="Say something about what you ate..."
            />
            <p className={styles.helper}>{foodWordCount} / 100 words</p>
          </div>

          <div className={styles.actions}>
            <button
              className={styles.primaryBtn}
              onClick={handleFoodPostUpload}
              disabled={uploadingFood}
            >
              {uploadingFood ? "Uploading Food..." : "Upload Food Post"}
            </button>
          </div>

          {foodPosts.length > 0 ? (
            <div className={styles.foodPreviewGrid}>
              {foodPosts.map((food) => (
                <article key={food.id} className={styles.foodPreviewCard}>
                  <div className={styles.foodPreviewImageWrap}>
                    <img
                      src={food.imageUrl}
                      alt={food.caption || "Food post"}
                      className={styles.foodPreviewImage}
                    />
                  </div>

                  <div className={styles.foodPreviewBody}>
                    <p className={styles.foodPreviewCaption}>{food.caption}</p>
                    ...
                    <button
                      ...
                      onClick={() => handleDeleteFoodPost(food.id)}
                    >
                      Delete Post
                    </button>
                  </div>
            ...
            <p className={styles.helper}>No food posts yet.</p>
          )}
        </section>
